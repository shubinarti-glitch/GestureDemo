...
/**
 * §4 ЛР: точное распознавание onPress / onTap / onDoubleTap / onLongPress
 * через pointerInput + detectTapGestures.
 */
@Composable
...
/**
 * §8 ЛР: verticalScroll + horizontalScroll + Canvas с большим изображением.
 * Box 150x150 dp служит «окном просмотра», Canvas 360x270 dp с картинкой
 * прокручивается внутри него по обеим осям.
 */
@Composable
...
/**
 * §9 ЛР: распознавание жеста сжатия (pinch) через Modifier.transformable.
 * rememberTransformableState лямбда получает три параметра — здесь используем
 * только scaleChange; rotationChange и offsetChange обязательны в сигнатуре.
 * Изменение размера применяется через graphicsLayer(scaleX, scaleY).
 *
 * Для эмулятора: Ctrl (Cmd на Mac) + drag = имитация мультитача.
 */
@Composable
fun MultiTouchDemo(modifier: Modifier = Modifier) {
    var scale by remember { mutableStateOf(1f) }
    var angle by remember { mutableStateOf(0f) }
    // §11 ЛР: добавляем translation (перемещение) — третий тип трансформации.
